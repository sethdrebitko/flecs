    ...
    _ table: UnsafePointer<ecs_table_t>)
{
    ...
    ecs_map_init_if(&cr.pointee.pair!.pointee.children_tables, &world.pointee.allocator)

    let elem = ecs_map_ensure(&cr.pointee.pair!.pointee.children_tables, table.pointee.id)
    ...
    if pr.pointee.count == 0 {
        ...
        if (table.pointee.flags & EcsTableIsDisabled) != 0 {
            cr.pointee.pair!.pointee.disabled_tables += 1
        }
        if (table.pointee.flags & EcsTableIsPrefab) != 0 {
            cr.pointee.pair!.pointee.prefab_tables += 1
        }
    } else {
        pr.pointee.entity = 0
    }
...
{
    if cr.pointee.pair == nil { return }
    let elem = flecs_component_get_parent_record(cr, table)
    if elem == nil { return }

    elem!.pointee.count -= 1

    if elem!.pointee.count == 0 {
        ecs_map_remove(&cr.pointee.pair!.pointee.children_tables, table.pointee.id)
        if (table.pointee.flags & EcsTableIsDisabled) != 0 {
            cr.pointee.pair!.pointee.disabled_tables -= 1
        }
        if (table.pointee.flags & EcsTableIsPrefab) != 0 {
...
{
    if r.pointee.table == nil { return -1 }
    ...
    if !ecs_is_alive(world, parent) { return -1 }
    ...
    let r_parent = flecs_entities_get(UnsafePointer(world), parent)
    if r_parent != nil {
        if r_parent!.pointee.table != nil {
            if (r_parent!.pointee.table!.pointee.flags & EcsTableIsPrefab) != 0 {
    ...
    _ entity: ecs_entity_t)
{
    ...
    let cr = flecs_components_get(UnsafePointer(world),
    ...
    flecs_remove_non_fragmenting_child_from_table(cr!, UnsafePointer(r!.pointee.table!))
}
...
public func flecs_on_non_fragmenting_child_move_add(
    ...
    _ count: Int32)
{
    if dst.pointee.data.entities == nil { return }
    let entities = dst.pointee.data.entities!
    let parents = ecs_table_get(world, dst, EcsParent.self, 0)
    ...
    for i in Int(row)..<Int(row + count) {
        ...
        let cr = flecs_components_get(UnsafePointer(world), ecs_childof(p))
        if cr == nil { continue }

        if src != nil && (src!.pointee.flags & EcsTableHasParent) != 0 {
...
{
    if src.pointee.data.entities == nil { return }
    let entities = src.pointee.data.entities!
    let parents = ecs_table_get(world, src, EcsParent.self, 0)
    if parents == nil { return }
    ...
    for i in Int(row)..<Int(row + count) {
        ...
        if dst != nil && (dst!.pointee.flags & EcsTableHasParent) != 0 {
            if update_parent_records {
                flecs_add_non_fragmenting_child_to_table(world, cr, e, dst!)
            }
        } else {
    ...
    _ count: Int32)
{
    ...
    for i in Int(row)..<Int(row + count) {
        let e = entities[i]
        let cr = flecs_components_get(UnsafePointer(world), ecs_childof(e))
        if cr == nil { continue }
...
{
    if (cr.pointee.flags & EcsIdOrderedChildren) != 0 {
        if cr.pointee.pair == nil { return false }
        return ecs_map_count(&cr.pointee.pair!.pointee.children_tables) != 0
    }
    return false
}
    ...
    _ cr: UnsafeMutablePointer<ecs_component_record_t>)
{
    ...
    ecs_vec_init(&world.pointee.allocator, &cr.pointee.pair!.pointee.ordered_children, elem_size, 0)
}
    ...
    _ cr: UnsafeMutablePointer<ecs_component_record_t>)
{
    if cr.pointee.pair == nil { return }
    let elem_size = Int32(MemoryLayout<ecs_entity_t>.stride)
    ecs_vec_fini(&world.pointee.allocator, &cr.pointee.pair!.pointee.ordered_children, elem_size)
    ecs_map_fini(&cr.pointee.pair!.pointee.children_tables)
}
    ...
    _ cr: UnsafeMutablePointer<ecs_component_record_t>)
{
    ...
    if (cr.pointee.flags & EcsIdMarkedForDelete) == 0 {
        ecs_vec_clear(&cr.pointee.pair!.pointee.ordered_children)
    }
}
    ...
    _ e: ecs_entity_t)
{
    ...
    let ptr = ecs_vec_append(&world.pointee.allocator,
        &cr.pointee.pair!.pointee.ordered_children, elem_size)?
    ...
    _ e: ecs_entity_t)
{
    ...
    let count = ecs_vec_count(&cr.pointee.pair!.pointee.ordered_children)
    if count <= 0 { return }
    let entities = ecs_vec_first(&cr.pointee.pair!.pointee.ordered_children)?
    ...
    for i in 0..<Int(count) {
        if entities![i] == e {
            ...
            cr.pointee.pair!.pointee.ordered_children.count -= 1
            break
        }
    }
}
    ...
    _ count: Int32)
{
    ...
    if src != nil && (src!.pointee.flags & EcsTableHasOrderedChildren) != 0 {
        let cr = flecs_table_get_childof_cr(world, src!)
        if cr == nil { return }
        if src!.pointee.data.entities == nil { return }
        let entities = src!.pointee.data.entities!
        for i in Int(row)..<Int(row + count) {
    ...
    if (dst.pointee.flags & EcsTableHasOrderedChildren) != 0 {
        let cr = flecs_table_get_childof_cr(world, dst)
        if cr == nil { return }
        if dst.pointee.data.entities == nil { return }
        let entities = dst.pointee.data.entities!
        for i in Int(row)..<Int(row + count) {
