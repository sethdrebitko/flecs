    ...
    _ id_out: UnsafeMutablePointer<ecs_id_t>?) -> Int32
{
    ...
    if table.pointee.type.array == nil { return -1 }
    let count = table.pointee.type.count
    let target_id = cr.pointee.id

    for i in 0..<Int(count) {
        if ecs_id_match(table.pointee.type.array![i], target_id) {
            id_out?.pointee = table.pointee.type.array![i]
            return Int32(i)
        }
    }
    return -1
    ...
    _ id_out: UnsafeMutablePointer<ecs_id_t>?) -> Int32
{
    ...
    while off < count {
        let type_id = table.pointee.type.array![Int(off)]
        off += 1
    ...
    _ id: ecs_id_t) -> Bool
{
    ...
    if (cr.pointee.flags & EcsIdExclusive) != 0 {
        if ECS_HAS_ID_FLAG(id, ECS_PAIR) {
            ...
            let cr_wc = flecs_components_get(world, ecs_pair(er, EcsWildcard))
            if cr_wc != nil {
...
public func ecs_search_relation_for_entity(
    ...
    _ id_out: UnsafeMutablePointer<ecs_id_t>?) -> Int32
{
    let r = flecs_entities_get(world, entity)
    if r == nil { return -1 }
    if r!.pointee.table == nil { return -1 }
    let table = r!.pointee.table!.assumingMemoryBound(to: ecs_table_t.self)
    ...
    if result != -1 {
        if tgt_out != nil && tgt_out!.pointee == 0 {
    ...
    _ table: UnsafePointer<ecs_table_t>) -> Int32
{
    if r == EcsChildOf {
        if (table.pointee.flags & EcsTableHasChildOf) != 0 {
            ...
            if world.pointee.cr_childof_wildcard != nil {
                let tr_wc = flecs_component_get_table(UnsafePointer(world.pointee.cr_childof_wildcard!), table)
                if tr_wc != nil {
    ...
    let cr = flecs_components_get(world, ecs_pair(r, EcsWildcard))
    if cr == nil {
        return 0
    }
...
private func flecs_relation_depth_walk(
    ...
    _ table: UnsafePointer<ecs_table_t>) -> Int32
{
    let tr = flecs_component_get_table(cr, table)
    if tr == nil { return 0 }
